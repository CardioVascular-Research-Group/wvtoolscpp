...
void WvToolsFacade::tsdb_upload(const std::string &prefix, const unsigned int &channel, const std::string &svm, const std::string &qrs_file, const std::string &tsdb_root, const int& chunk_size) {
    try {
        InfoReader info_reader(prefix);
        WvReader wv_reader(prefix);
        ...
        TsdbUploader tsdb_uploader((unsigned int)chunk_size, tsdb_root);
...
void WvToolsFacade::tsdb_check(const std::string &prefix, const string& tsdb_root) {
    try {
        ...
        for (unsigned int c = 0; c < info_reader.num_channels(); c++) {
            if (tsdb_checker.check_presence(query_converter.metrics[c], prefix, (unsigned long)timestamp_calculator.calculate_timestamp(timestamp_calculator.calculate_offset(0)))) {
                if (c + 1 < info_reader.num_channels()) {
                    cout << 1 << "\t";
                } else {
                    cout << 1 << endl;
